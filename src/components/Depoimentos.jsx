...
export default function Depoimentos() {
  ...
  useEffect(() => {
    const fetchDepoimentos = async () => {
      ...
      setDepoimentos(data);
      setCurrentPage(0); // Resetar quando carregar
    };
  ...
  const paginated = depoimentos.slice(
  ...
  return (
    <section className="bg-[#FFD675] py-16 px-6 md:px-10 text-black pt-[90px] pb-[35px]">
                  ...
                  >
                    {(dep.nota && dep.tituloNota) && (
                      <div className="mb-4 flex items-center gap-2">
                        ...
                        <div className="text-[#FFD675] font-semibold text-sm">
                          {dep.tituloNota}
                        </div>
                      </div>
                    )}
...
<button
  ...
  className="bg-black bg-opacity-70 p-2 rounded-full text-white disabled:opacity-50 hover:bg-opacity-90"
>
...
>
  <FaChevronRight size={20} />
                ...
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    className={`h-3 w-3 rounded-full transition-all ${
                      currentPage === i ? "bg-[#0F0A0B]" : "bg-[#0F0A0B]/40"
                    }`}
        ...
        {/* Formulário */}
        <div
          className={`transition-all duration-700 overflow-hidden mt-10 ${
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full p-3 border border-white bg-[#0F0A0B] text-white rounded-lg focus:ring-2 focus:ring-white outline-none"
              />
              <h4 className="text-lg font-semibold text-[#FFD675] mt-2">Avaliar</h4>
              <select
                name="nota"
                value={form.nota}
                onChange={handleChange}
            ...
            {statusMsg && (
              <p
                className={`mt-6 text-center ${
                  statusMsg.includes("erro") ? "text-red-500" : "text-green-500"
                }`}
              >
                {statusMsg}
              </p>
